...
export default ({ submit }) => {
  ...
  const { addDrawerShow, toggleAddDrawerShow, novelFormData, setNovelFormData } = useStore()
  const { tabs, initTabs, novelData, setNovelStore } = useViewDataStore()
  ...
  const formfinish = async () => {
    ...
    if (novelFormData.action === 'add') {
      ...
      setNovelStore({
        novelList: [body].concat(novelData),
      })
    } else {
      const index = novelData.findIndex(fv => fv['_id'] === body['_id'])
      novelData[index] = body
      setNovelStore({ novelList: [].concat(novelData) })
    }
