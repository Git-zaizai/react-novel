  ...
  return (
    <>
      ...
      <div className={styles.wola}>
        ...
        <Modal open={modelOpen} onOk={bindfreedMap} onCancel={setModelOpen} centered confirmLoading={confirmLoading} title='请确认销毁'>
      ...
      >
        <App message={{ top: 70 }}>
          ...
          <Layout className={styles.wolLayout}>
            ...
            >
              <Form.Item name='wssid' label='id：（最好使用英文和数字组合，自己记得就好）' rules={[rulesID]} validateTrigger='onBlur'>
              ...
              {items.length > 0 &&
                items.map((item, index) => (
                  <div className={styles.items} key={item.uuid}>
                    ...
                    <Button type={item.buttype} block danger className='mt-10' loading={item.loading} onClick={() => binditemClick(item, index)}>
                      唤醒
                    </Button>
                    <div className='flex-ai-c mt-10'>
                      <Input placeholder='随便写-测试看看在线不' allowClear onInput={e => setTestsendValue(e.target.value)} />
